...
function prospectHtml(p: BookingEmailPayload): string {
  const rescheduleBanner = p.isReschedule
    ...
    : "";
  const headerLabel = p.isReschedule ? "Demo Updated" : "Demo Confirmed";
  const headerTitle = p.isReschedule
    ? `Your demo is now on ${formatDate(p.date)}`
    : `You're booked in, ${p.name.split(" ")[0]}!`;
  const headerSub = p.isReschedule
    ? "Your ServiceSupport.UK demo has been moved to a new time."
    : "Your ServiceSupport.UK demo is confirmed.";
  const oldInfo = p.isReschedule && p.oldDate
    ? `<tr><td colspan="2" style="padding-bottom:14px;">
        <div style="color:#64748b;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px;">Previous Time</div>
        <div style="color:#94a3b8;font-size:13px;font-weight:600;text-decoration:line-through;">${formatDate(p.oldDate)} at ${p.oldTime}</div>
       </td></tr>`
    : "";
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;padding:40px 20px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        ${rescheduleBanner}
        <tr><td style="background:#134e4a;${p.isReschedule ? "" : "border-radius:16px 16px 0 0;"}padding:32px 36px 28px;">
          <div style="color:#5eead4;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;">${headerLabel}</div>
          <div style="color:#ffffff;font-size:26px;font-weight:900;line-height:1.2;">${headerTitle}</div>
          <div style="color:#99f6e4;font-size:14px;margin-top:8px;">${headerSub}</div>
        </td></tr>
        ...
        <tr><td style="background:#1e293b;padding:28px 36px;">
          ...
          ${p.videoLink ? `<div style="margin-top:24px;text-align:center;">
...
function adminHtml(p: BookingEmailPayload): string {
  ...
  const oldInfo = p.isReschedule && p.oldDate
    ? `<tr><td style="border-top:1px solid #1e293b;padding-top:10px;padding-bottom:10px;">
        <div style="color:#64748b;font-size:10px;text-transform:uppercase;letter-spacing:1px;margin-bottom:2px;">Previous Time</div>
        <div style="color:#94a3b8;font-size:13px;text-decoration:line-through;">${formatDate(p.oldDate)} at ${p.oldTime}</div>
       </td></tr>`
    : "";
  return `<!DOCTYPE html>
<html>
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:500px;">
        <tr><td style="background:#1e293b;border:1px solid #334155;border-radius:16px;padding:28px 32px;">
          <div style="color:${p.isReschedule ? "#5eead4" : "#fbbf24"};font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;">${label}</div>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f172a;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
            <tr><td style="padding-bottom:10px;">
              <div style="color:#64748b;font-size:10px;text-transform:uppercase;letter-spacing:1px;margin-bottom:2px;">New Date &amp; Time</div>
            ...
            ${p.videoLink ? `<tr><td style="border-top:1px solid #1e293b;padding-top:10px;">
          ...
          ${p.videoLink ? `<a href="${p.videoLink}" style="display:block;background:#14b8a6;color:#ffffff;font-size:14px;font-weight:900;text-decoration:none;border-radius:10px;padding:12px 24px;text-align:center;">
            Join Meeting
          </a>` : ""}
        </td></tr>
      </table>
