-- reschedule_meeting: frees the old slot(s) and claims a new one for an existing booking.
-- Handles on-site buffer logic (blocks 5-hour window if switching to/keeping on-site).
-- Returns the new slot details so the caller can send a reschedule email.
...
BEGIN
  ...
  -- Get the current booking's meeting type and video link
  ...
  INTO v_new_date, v_new_time, v_new_duration
  ...
  WHERE booked_by_booking_id = p_booking_id
    AND notes NOT LIKE '%[blocked: on-site buffer]%'
  ...
  SET booked = false,
      ...
      notes = regexp_replace(notes, ' ?\[blocked: on-site buffer\]', '')
  WHERE booked_by_booking_id = p_booking_id;
  ...
  IF p_meeting_type = 'onsite' THEN
    ...
    -- Check max 2 onsite per day (excluding this booking's old slots, already freed)
    ...
    IF v_onsite_count >= 2 THEN
      -- Re-book the old slots since we're aborting
      UPDATE demo_availability
    ...
    -- Check for conflicts in the 5-hour window
    FOR v_conflict IN
    ...
    LOOP
      ...
      RETURN jsonb_build_object('ok', false, 'error',
        format('Conflict: slot %s is already booked by %s (%s)',
  ...
  -- Update the booking with new meeting details
  v_video_link := CASE WHEN p_meeting_type = 'virtual' THEN COALESCE(v_video_link, 'https://meet.google.com/mav-hmei-vzi') ELSE '' END;
