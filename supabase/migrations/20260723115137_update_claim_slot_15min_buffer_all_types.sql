-- Update claim_slot to enforce a 15-minute buffer either side of EVERY booked meeting
-- (both virtual and onsite). Onsite keeps its additional 2hr travel buffer.
...
BEGIN
  ...
  v_buffer_end   := v_meeting_start + 60 + 15;  -- meeting duration (60) + 15 min after

  -- Onsite meetings get an additional travel buffer (2hr before, 3hr after)
  ...
  LOOP
    ...
    SET booked = true,
        ...
        notes = COALESCE(notes, '') || ' [blocked: buffer]'
    WHERE id = v_block.id;
  END LOOP;
  ...
  IF FOUND AND v_booking.sourced_by_user_id IS NOT NULL THEN
    ...
    -- If a deal_id was passed in, link the booking to it and skip deal creation
    ...
    ELSE
      ...
      -- Compute org_key (matches the generated column expression)
      v_org_key := lower(regexp_replace(COALESCE(v_booking.business_name, ''), '\s+', '', 'g')) || '|' ||
      lower(regexp_replace(COALESCE(v_booking.postcode, ''), '\s+', '', 'g'));

      -- Find or create the organisation (org_key is generated, don't insert it)
