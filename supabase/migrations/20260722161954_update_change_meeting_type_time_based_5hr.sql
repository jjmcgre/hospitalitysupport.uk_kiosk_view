-- Update change_meeting_type: block 5 hours by time, with conflict detection
...
BEGIN
  ...
  IF p_new_type = 'onsite' THEN
    ...
    -- Check max 2 onsite per day (excluding this booking)
    ...
    -- Check for conflicts: any slot in the 5-hour window booked by another booking
    FOR v_conflict IN
    ...
    -- Block all free slots in the 5-hour window (except the meeting slot)
    FOR v_block IN
