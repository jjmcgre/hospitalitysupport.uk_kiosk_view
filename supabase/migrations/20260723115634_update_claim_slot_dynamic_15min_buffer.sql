-- claim_slot now works with 15-minute grid slots.
-- When a slot is picked, the system dynamically blocks:
--   - 15 minutes before the meeting (pre-buffer)
--   - The meeting itself (60 minutes)
--   - 15 minutes after the meeting (post-buffer)
-- For onsite meetings, the travel buffer (2hr before, 3hr after) still applies.
