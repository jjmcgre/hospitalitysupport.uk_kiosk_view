...
export default function ChatPage() {
  ...
  async function loadData() {
    ...
    const [msgRes, memRes] = await Promise.all([
      ...
      supabase.from('user_profiles').select('auth_user_id, display_name, role').eq('is_active', true).order('display_name'),
    ]);
    setMessages((msgRes.data ?? []) as Message[]);
    setMembers((memRes.data ?? []).map((m: { auth_user_id: string; display_name: string; role: string }) => ({ id: m.auth_user_id, display_name: m.display_name, role: m.role })) as TeamMember[]);
    setLoading(false);
  }
